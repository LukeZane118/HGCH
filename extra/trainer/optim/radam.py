...
import torch.optim
from extra.model.hgcn.manifolds import ManifoldParameter, Euclidean
            ...
            for group in self.param_groups:
                ...
                for point in group["params"]:
                    ...
                    exp_avg_sq.mul_(betas[1]).add_(
                            manifold.component_inner(point, c, grad, keepdim=True),
                            alpha=1 - betas[1]
                    )
                    group["step"] += 1
                    ...
                    new_point = manifold.expmap(-step_size * direction, point, c)
                    exp_avg_new = manifold.ptransp(point, new_point, exp_avg, c)
                    ### debug code
                    # if torch.any(torch.isnan(new_point)):
                    #     raise ValueError('new_point has nan.')
                    # if torch.any(torch.isnan(exp_avg_new)):
                    #     raise ValueError('exp_avg_new has nan.')
                    ###
