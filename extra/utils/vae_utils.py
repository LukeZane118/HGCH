...
from ..model.vae.components import Component, ConstantComponent, EuclideanComponent, StereographicallyProjectedSphereComponent
from ..model.vae.components import SphericalComponent, UniversalComponent, HyperbolicComponent, PoincareComponent
from ..model.vae.sampling import WrappedNormalProcedure, EuclideanNormalProcedure, EuclideanConstantProcedure
from ..model.vae.sampling import UniversalSamplingProcedure
...
def parse_components(arg: str, fixed_curvature: bool, radius=5.0) -> List[Component]:
    """
    ...
    def _create_space(space_multiplier: int, space_type: str, dim: int,
        ...
        for _ in range(space_multiplier):
            if space_type in ["h", "p"]:
                yield space_creator(dim, fixed_curvature, 
                                    sampling_procedure=sampling_procedure_map[space_creator], radius=radius)
            else:
                yield space_creator(dim, fixed_curvature, 
                                    sampling_procedure=sampling_procedure_map[space_creator])
