...
class Sampler(AbstractSampler):
    ...
    def __init__(self, phases, datasets, distribution='uniform'):
        ...
        self.user_num = datasets[0].user_num    # logic of user_num
...
class HeldoutSampler(Sampler):
    """:class:`HeldoutSampler` is used to sample negative items for each input user. In order to avoid positive items
    in train-phase to be sampled in valid-phase, and positive items in train-phase or valid-phase to be sampled
    in test-phase, we need to input the datasets of all phases for pre-processing. And, before using this sampler,
    it is needed to call :meth:`set_phase` to get the sampler of corresponding phase.
    ...
    def __init__(self, phases, datasets, distribution='uniform'):
        assert len(phases) == 3
        super().__init__(phases, datasets, distribution=distribution)
    ...
    def get_used_ids(self):
        ...
        used_item_id = dict()
        # last = [set() for _ in range(self.user_num)]
        for phase, dataset in zip(self.phases[:1], self.datasets[:1]):
            cur = np.array([set() for _ in range(self.user_num)])
            for uid, iid in zip(dataset.inter_feat[self.uid_field].numpy(), dataset.inter_feat[self.iid_field].numpy()):
                cur[uid].add(iid)
            used_item_id[phase] = cur
            
        for phase, datasets in zip(self.phases[1:], self.datasets[1:]):
            cur = np.array([set() for _ in range(self.user_num)])
            for ds in datasets:
                # cur = np.array([set(s) for s in last])
                for uid, iid in zip(ds.inter_feat[self.uid_field].numpy(), ds.inter_feat[self.iid_field].numpy()):
                    cur[uid].add(iid)
            used_item_id[phase] = cur
