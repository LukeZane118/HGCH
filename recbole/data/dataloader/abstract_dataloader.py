...
class NegSampleDataLoader(AbstractDataLoader):
    ...
    def _neg_sampling(self, inter_feat):
        if 'hard' in self.neg_sample_args.keys() and self.neg_sample_args['hard'] != 'none':
            hard_sample_num = self.neg_sample_args['hard']
            assert hard_sample_num <= self.neg_sample_num
            easy_sample_num = self.neg_sample_num - hard_sample_num
            # hard negtive
            ...
            neg_candidate_ids = self.sampler.sample_by_user_ids(user_ids, item_ids, hard_sample_num * candidate_num + easy_sample_num)
            hard_neg_candidate_ids, easy_neg_item_ids = torch.split(neg_candidate_ids, [hard_sample_num * candidate_num * len(user_ids), easy_sample_num * len(user_ids)])
            self.model.eval()
            interaction = copy.deepcopy(inter_feat).to(self.model.device)
            interaction = interaction.repeat(hard_sample_num * candidate_num)
            neg_item_feat = Interaction({self.iid_field: hard_neg_candidate_ids.to(self.model.device)})
            ...
            hard_neg_candidate_ids = hard_neg_candidate_ids.reshape(candidate_num, -1)
            hard_neg_item_ids = hard_neg_candidate_ids[indices, [i for i in range(hard_neg_candidate_ids.shape[1])]].view(-1)
            self.model.train()
            # easy negtive
            # easy_neg_item_ids = self.sampler.sample_by_user_ids(user_ids, item_ids, easy_sample_num)
            neg_item_ids = torch.cat([hard_neg_item_ids, easy_neg_item_ids])
            return self.sampling_func(inter_feat, neg_item_ids)
        elif 'dynamic' in self.neg_sample_args.keys() and self.neg_sample_args['dynamic'] != 'none':
            candidate_num = self.neg_sample_args['dynamic']
            user_ids = inter_feat[self.uid_field].numpy()
            item_ids = inter_feat[self.iid_field].numpy()
