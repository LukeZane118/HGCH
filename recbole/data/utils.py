...
from recbole.sampler import KGSampler, Sampler, RepeatableSampler, HeldoutSampler
...
def data_preparation(config, dataset):
    ...
    if dataloaders is not None:
        ...
        if isinstance(test_dataset, tuple):
            train_sampler, valid_sampler, test_sampler = create_heldout_samplers(config, dataset, built_datasets)
...
def get_dataloader(config, phase):
    ...
    register_table = {
        ...
        'HVAE': _get_AE_dataloader,
        'MultiVAESSL': _get_HO_dataloader,
    }
...
def _get_HO_dataloader(config, phase):
...
def create_heldout_samplers(config, dataset, built_datasets):
    """Create sampler for training, validation and testing.
    ...
    #     else:
    #         sampler = RepeatableSampler(phases, dataset, train_neg_sample_args['distribution'])
    sampler = HeldoutSampler(phases, built_datasets)
    train_sampler = sampler.set_phase('train')

    if eval_neg_sample_args['strategy'] != 'none':
        if sampler is None:
            if not config['repeatable']:
                sampler = HeldoutSampler(phases, built_datasets, eval_neg_sample_args['distribution'])
            else:
                sampler = RepeatableSampler(phases, dataset, eval_neg_sample_args['distribution'])
        else:
            sampler.set_distribution(eval_neg_sample_args['distribution'])
